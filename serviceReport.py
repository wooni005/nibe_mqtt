...
def on_message_check(client, userdata, msgJson):
    if (current_sec_time() - systemWatchTimer) > 1800:
        sendCheckReportToHomeLogic(True, ACTION_RESTART, "Timeout systemWatchTimer, 30 min no activity in Run-thread loop")
    else:
